...
#Perform searches
echo "[i] Firmware Directory" | tee -a $file
echo $FIRMDIR | tee -a $file
echo "[i] Search for passwd and shadow files" | tee -a $file
...
find $FIRMDIR -name "passwd-" | cut -c${#FIRMDIR}- | tee -a $file
echo | tee -a $file
echo "[i] List etc/ssl directory" | tee -a $file
echo " [-] etc/ssl" | tee -a $file
ls -l $FIRMDIR/etc/ssl | tee -a $file
echo | tee -a $file
echo "[i] Search for SSH authorized_keys file" | tee -a $file
echo " [-] SSH" | tee -a $file
find $FIRMDIR -name "authorized_keys" | cut -c${#FIRMDIR}- | tee -a $file
echo | tee -a $file
echo "[i] Search for configuration files" | tee -a $file
echo " [-] configuration files (.conf or .cfg or .config)" | tee -a $file
...
echo | tee -a $file
echo "[i] Search for SSL related files" | tee -a $file
...
echo | tee -a $file
echo "[i] Search for web servers" | tee -a $file
